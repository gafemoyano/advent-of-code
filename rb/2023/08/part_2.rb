...
guide = map.split("\n").each_with_object({}) do |line, memo|
  location, left, right = line.scan(/([A-Z0-9]{3})/).map(&:first)

  memo[location] = [left, right]
end
pp guide

next_locations = guide.keys.filter { |key| key.end_with?("A") }

instructions = instructions.chars

next_locations.map do |curr|
  i = 0

  loop do
    break if curr.end_with?("Z")

    step = instructions[i % instructions.length]
    direction = (step == "L") ? 0 : 1

    curr = guide[curr][direction]
    i += 1
  end
  i
end.inject(:lcm).tap { pp _1 }
